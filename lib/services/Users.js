...
const UserNaturalPut = require('../models/UserNaturalPut');
...
var Users = Service.extend({
    ...
    create: function(user, callback, options) {
        ...
        // user.Sca is needed for SCA when using TS, but it should not be sent to the API
        delete user.Sca;
     ...
     * @param {Object|UserNatural|UserLegal|UserNaturalSca|CreateUserNaturalScaPayerData}  user
     */
    _getUserApiAndClass: function(user) {
        // user.Sca is needed when using TS
        if (user instanceof UserNaturalSca || user.Sca === true) {
            return {
